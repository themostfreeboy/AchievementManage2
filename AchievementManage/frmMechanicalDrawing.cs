...
namespace AchievementManage
{
    public partial class frmMechanicalDrawing : Form
    {
        ...
        private void dgvData_CellClick(object sender, DataGridViewCellEventArgs e)//单击单元格的任意部分
        {
            ...
            this.txtAchievementName.Text = this.dgvData.Rows[index].Cells[0].Value.ToString().Trim();
            if (txtAchievementName.Text != string.Empty)//点中了非最后一行的空白行
            {
                this.cboSoftware.Text = this.dgvData.Rows[index].Cells[6].Value.ToString().Trim();
                this.txtSaveDirectory.Text = this.dgvData.Rows[index].Cells[7].Value.ToString().Trim();
                this.txtPartNum.Text = this.dgvData.Rows[index].Cells[8].Value.ToString().Trim();
                this.txtAssemblyNum.Text = this.dgvData.Rows[index].Cells[9].Value.ToString().Trim();
                this.txtDrawingNum.Text = this.dgvData.Rows[index].Cells[10].Value.ToString().Trim();
                btnUpdate.Enabled = true;//更新机械图信息按钮可点击
            }
            else//点中了最后一行的空白行
            {
        ...
        private void btnUpdate_Click(object sender, EventArgs e)//更新机械图信息
        {
            ...
            try
            {
                ...
                string sql_1 = string.Format("update mechanicaldrawing set mechanicaldrawing.AchievementName='{0}', mechanicaldrawing.Software='{1}', mechanicaldrawing.SaveDirectory='{2}', mechanicaldrawing.PartNum={3}, mechanicaldrawing.AssemblyNum={4}, mechanicaldrawing.DrawingNum={5} where mechanicaldrawing.AchievementName='{6}';", this.txtAchievementName.Text.ToString().Trim(), this.cboSoftware.Text.ToString().Trim(), this.txtSaveDirectory.Text.ToString().Trim(), Convert.ToInt32(this.txtPartNum.Text.ToString().Trim()), Convert.ToInt32(this.txtAssemblyNum.Text.ToString().Trim()), Convert.ToInt32(this.txtDrawingNum.Text.ToString().Trim()), this.txtAchievementName.Text.ToString().Trim());//sql语句
                if (MyDatabase.UpdateDataBySql(sql_1))//更新成功
                {
                    ...
                    DataSet ds_2 = MyDatabase.GetDataSetBySql(sql_2);
                    this.dgvData.DataSource = ds_2.Tables[0];//机械图信息显示DataGridView控件显示数据
                    btnUpdate.Enabled = true;//更新机械图信息按钮可点击
                    btnOutToFile.Text = "导出此视图内容到文件";//设置导出此视图内容到文件按钮显示内容
                    btnOutToFile.Enabled = true;//设置导出此视图内容到文件按钮可点击
